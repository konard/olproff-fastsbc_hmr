...
std::optional<Enum> lookup(const std::array<std::pair<std::string_view, Enum>, N>& table,
                           std::string_view s) {
...
std::optional<HeaderAction> AstFactory::headerAction(std::string_view s) {
...
std::optional<ElementAction> AstFactory::elementAction(std::string_view s) {
    static constexpr std::array table{
...
std::optional<ElementType> AstFactory::elementType(std::string_view s) {
    static constexpr std::array table{
...
std::optional<MatchValType> AstFactory::matchValType(std::string_view s) {
    static constexpr std::array table{
...
std::optional<MsgType> AstFactory::msgType(std::string_view s) {
    static constexpr std::array table{
...
std::string_view AstFactory::toString(HeaderAction a) {
    switch (a) {
...
std::string_view AstFactory::toString(ElementAction a) {
    switch (a) {
...
std::string_view AstFactory::toString(ElementType t) {
    switch (t) {
...
std::string_view AstFactory::toString(ComparisonType c) {
    switch (c) {
...
std::string_view AstFactory::toString(MatchValType m) {
    switch (m) {
...
std::string_view AstFactory::toString(MsgType m) {
    switch (m) {
...
bool AstFactory::isBlockKeyword(std::string_view s) {
    std::string k = lower(s);
    return k == "sip-manipulation" || k == "header-rule" ||
           k == "header-rules" || k == "element-rule" || k == "element-rules";
}

std::vector<std::string> AstFactory::parseMethods(std::string_view s) {
