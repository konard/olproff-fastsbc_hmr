...
struct ValueRef {
    ...
    int captureIndex = -1;   // capture index for Capture, or trailing .$N
};

struct ValueSegment {
    bool isRef = false;
    std::string literal;  // valid when !isRef
    ValueRef ref;         // valid when isRef
};
...
public:
    ...
    [[nodiscard]] bool isPureLiteral() const;
    // Concatenated literal text (only meaningful when isPureLiteral()).
    [[nodiscard]] std::string literalText() const;
    // True when the value is exactly one reference and nothing else.
    [[nodiscard]] bool isSingleRef() const;

    // For pattern-rule match-values that are plain regexes (one literal segment
    // and not a $reference), this is the regex source.
    [[nodiscard]] bool isRegexLiteral() const;

private:
    std::string raw_;
    std::vector<ValueSegment> segments_;
    bool negated_ = false;
    bool refExpression_ = false;  // match-value that is a $reference, not regex
};
